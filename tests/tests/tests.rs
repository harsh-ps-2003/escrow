...
//     let fm_test_dir = env::var("FM_TEST_DIR").unwrap_or_else(|_|
// "/tmp/fedimint_test".to_string());     let fm_fed_size =
// env::var("FM_FED_SIZE").unwrap_or_else(|_| "3".to_string());

//     let globals =
//         vars::Global::new(Path::new(&fm_test_dir),
// fm_fed_size.parse::<usize>()?, 0).await?;     let log_file =
...
//         writeln!(env_string, r#"export {var}="{value}""#)?; // hope that
// value doesn't contain a "         std::env::set_var(var, value);
//     }
//     write_overwrite_async(globals.FM_TEST_DIR.join("env"),
// env_string).await?;     info!("Test setup in {:?}", globals.FM_DATA_DIR);
...
//     info!("Federation setup complete. Waiting for nodes to be ready...");
...
//     // let arbiter_pubkey = cmd!(
//     //     "fedimint-cli-arbiter",
//     //     "module",
//     //     "escrow",
//     //     "public-key"
//     // )
//     // .out_json()
//     // .await?;
//     // let arbiter_publickey =
// arbiter_pubkey["public_key"].as_str().unwrap();
